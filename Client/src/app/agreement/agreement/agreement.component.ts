...
import { HTTP_SUCCESS_STATUS, HTTP_ERROR_STATUS, COMMON_APPROVE_LABEL,COMMON_RETURN_LABEL } from '../../app-constants';
...
export class AgreementComponent implements OnInit, OnDestroy {
  ...
  constructor(private _autoSaveService: AutoSaveService, private _route: ActivatedRoute,
  ...
  */
  ...
  triggerSubmitModal() {
	...
	invokeBusinessRule() {
		...
		if (!this.isSaving) {
           ...
           this._agreementService.getApplicableQuestionnaire(this.getQuestionnaireRequestObject())).subscribe(async (data: any) => {
				...
				this.checkQuestionnaireCompletion(data[1]);
				this.isSaving = false;
	...
	checkQuestionnaireCompletion(data) {
		const APPLICABLE_QUESTIONNAIRE = data.applicableQuestionnaire;
		const INCOMPLETE_MANDATORY_QUESTION = this.checkForAnyIncompleteMandatoryQuestion(APPLICABLE_QUESTIONNAIRE);
		this.setValidationMsgArray(INCOMPLETE_MANDATORY_QUESTION);
	}

	checkForAnyIncompleteMandatoryQuestion(applicableQuestionnaire: any) {
		return applicableQuestionnaire.find(question => question.IS_MANDATORY === 'Y' &&
			(question.QUESTIONNAIRE_COMPLETED_FLAG === 'N' || question.QUESTIONNAIRE_COMPLETED_FLAG == null));
	}

	setValidationMsgArray(incompleteQuestion: any) {
		if (incompleteQuestion) {
			const element = { validationType: 'VE', validationMessage: 'Questionnaire not completed.' };
  ...
  getDataAfterSubmission() {
    this.$subscriptions.push(this._agreementService.submitAgreement(this.submitAgreementObject)
      .subscribe((data: any) => {
        ...
        this._autoSaveService.errors = [];
        this.updateAgreementStoreData();
        this._commonAgreementData.isAgreementDataChange = false;
        this._commonService.showToast(HTTP_SUCCESS_STATUS, 'Agreement Submitted successfully');
      },
        err => { this._commonService.showToast(HTTP_ERROR_STATUS, 'Submitting Agreement failed. Please try again.'); },
      ));
  }
      ...
      this.$subscriptions.push(this._agreementService.maintainAgreementWorkFlow
        (this.requestObject, this.uploadedFile).subscribe((data: any) => {
          ...
          if (this.requestObject.actionType === 'A') {
            ...
            this._commonService.showToast(HTTP_SUCCESS_STATUS, `Agreement ${COMMON_RETURN_LABEL.toLowerCase()}d successfully.`);
          }
