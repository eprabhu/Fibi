...
export class GrantCallComponent implements OnInit, OnDestroy {
  ...
  isEligibleToCreateProposal = false;
  ...
  ngOnInit() {
    this.getPermissions();
    this.result = this._commonData.$grantCallData.value;
    ...
    this._commonData.grantCallTitle.abbrevation = this.result.grantCall.abbrevation;
    document.getElementById('scroll-top').scrollIntoView({ block: 'end' });
  }

  triggerGrantCallDetails() {
    this.$subscriptions.push(this._grantService.isGrantActive.subscribe(data => {
  ...
  ngOnDestroy() {
    ...
    this._commonData.$isMandatoryFilled.next(false);
    this._commonData.grantCallTitle = { title: '', abbrevation: '' };
  }
  ...
  initialLoad() {
    ...
    this._commonData.getGrantCallMode();
  }

  /**fetches grant details for the given Id used in refernce with copy grant */
  loadGrantById() {
    this.$subscriptions.push(this._grantService.loadGrantById({
  ...
  openGoBackModal() {
    ...
    if (this.isModifyGrantCall && this.result.grantCall.grantStatusCode !== 5 && this._commonData.isGrantCallDataChange) {
  ...
  checkGrantCallPublish() {
    if (this._commonData.isGrantCallDataChange) {
      this._grantService.isSaveGrantcall.next(true);
      if (!this._commonData.errorMessage) {
        this.saveGrantCallForRuleEvaluation();
      }
    }
    if (!this._commonData.errorMessage || !this._commonData.isGrantCallDataChange) {
      this.getGrantCallGeneralData();
      this.checkMainPannelStatus();
      this.modalHideAndShowObject.isShowPublishGrantCallWarning = false;
      if (this.result.grantCall.grantStatusCode === 1 &&
        ((this.result.grantCall.grantCallType.categoryCode !== 1 && this._commonService.isGrantCallStatusAutomated)
          || !this._commonService.isGrantCallStatusAutomated)) {
  ...
  saveGrantCallForRuleEvaluation() {
    const previousMode = this._commonData.isViewMode;
    this.$subscriptions.push(this._grantService.saveGrantCall({
      ...
      this._commonData.isViewMode = previousMode;
      this._commonData.isGrantCallDataChange = false;
    }));
  }
  ...
  publishGrantCall() {
    ...
    this.$subscriptions.push(this._grantService.publishGrantCall({ 'grantCall': this.result.grantCall }).subscribe((success: any) => {
      ...
      this._commonService.showToast(HTTP_SUCCESS_STATUS, 'Grant Call published successfully.');
      this._commonData.$isMandatoryFilled.next(false);
    }, err => {
      this._commonService.showToast(HTTP_ERROR_STATUS, 'Publishing Grant Call failed. Please try again.');
    }));
  }
  ...
  copyGrantCall() {
    ...
    this.$subscriptions.push(this._grantService.copyGrantCall({
      ...
      this._commonService.showToast(HTTP_ERROR_STATUS, 'Copying Grant Call failed. Please try again.');
    }));
  }
  ...
  */
  ...
  deleteGrantCall() {
    $('#warningOrAlertModal').modal('hide');
    this.modalHideAndShowObject.isShowGrantCallDeleteModal = false;
    this.$subscriptions.push(this._grantService.deleteGrantcall({ 'grantCallId': this.grantId }).subscribe((success: any) => {
      if (success.message === 'GrantCall deleted successfully') {
        this._router.navigate(['fibi/dashboard/grantCall']);
      }
    }, err => {
      this._commonService.showToast(HTTP_ERROR_STATUS, 'Deleting Grant Call failed. Please try again.');
    }));
  }

  archiveGrantCall(type) {
    $('#warningOrAlertModal').modal('hide');
    this.$subscriptions.push(this._grantService.archiveGrantCall({
      'updateUser': this._commonService.getCurrentUserDetail('userName'),
      'grantCallId': this.result.grantCall.grantCallId
    }).subscribe((data: any) => {
      this.result.grantCall = data.grantCall;
      ...
      if (type === 'archive') {
        this._commonService.showToast(HTTP_ERROR_STATUS, 'Archiving Grant Call failed. Please try again.');
      } else if (type === 'unarchive') {
        this._commonService.showToast(HTTP_ERROR_STATUS, 'Unarchiving Grant Call failed. Please try again.');
      }
    },
      () => {
        this._commonData.isGrantCallDataChange = false;
        if (type === 'archive') {
          this._commonService.showToast(HTTP_SUCCESS_STATUS, 'GrantCall archived successfully.');
        } else if (type === 'unarchive') {
          this._commonService.showToast(HTTP_SUCCESS_STATUS, 'GrantCall unarchived successfully.');
        }
      }));
  }
  ...
  modifyGrant() {
    this._commonData.isViewMode = false;
  ...
  async getPermissions() {
    ...
    this.isPublishGrantCall = await this._commonService.checkPermissionAllowed('PUBLISH_GRANT_CALL');
    this.isDeleteGrantCall = await this._commonService.checkPermissionAllowed('DELETE_GRANT_CALL');
    this.isArchiveGrantCall = await this._commonService.checkPermissionAllowed('ARCHIVE_GRANT_CALL');
  ...
  validateGrantCall() {
    ...
    this.debounceTimer = setTimeout(() => {
      this.$subscriptions.push(this._commonService.evaluateValidation(this.validationObject).subscribe((data: any) => {
  ...
  setValiadtionAndModalFlags() {
    if (this._grantService.isMandatoryFilled) {
      if (this.result.grantCall.grantStatusCode === 1 &&
        ((this.result.grantCall.grantCallType.categoryCode !== 1 && this._commonService.isGrantCallStatusAutomated)
          || !this._commonService.isGrantCallStatusAutomated)) {
        this.modalHideAndShowObject.isShowPublishGrantCallWarning = true;
 ...
 */
  saveAllProposals() {
    ...
    if (this.mainPanelReview.checkAll === false) {
      ...
      }, err => { this._commonService.showToast(HTTP_ERROR_STATUS, 'Save all proposals failed. Please try again.'); },
        () => {
          this._commonService.showToast(HTTP_SUCCESS_STATUS, 'Proposal details saved successfully.');
        }));
    }
  }
  ...
  */
  checkDeleteGrantCallOrNot(grantCallId) {
    ...
    this.$subscriptions.push(this._grantService.checkCanDeleteGrantCall({ 'grantCallId': this.grantId }).subscribe((response: any) => {
  ...
  */
  ...
  exportGrantCallEvaluationReport(exportIndex: string) {
    ...
    this.$subscriptions.push(this._grantService.exportGrantCallEvaluationReport(exportDataReqObject).subscribe(
      data => {
  ...
  submitIOI() {
    this._grantService.ioiSubmit$.next();
  }
}
