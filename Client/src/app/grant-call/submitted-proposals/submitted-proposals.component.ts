...
import { openInNewTab, fileDownloader } from '../../common/utilities/custom-utilities';
...
export class SubmittedProposalsComponent implements OnInit, OnDestroy {
  ...
  */
  ...
  saveReview(proposalStatus) {
    this.mainPanelObject.submittedProposals = this.selectedArray.map(({ isScoreInvalid, ...data }) => data);
    this.mainPanelObject.statusType = proposalStatus;
    this.mainPanelObject.updateUser = this._commonService.getCurrentUserDetail('userName');
    this.$subscriptions.push(this._grantService.saveorUpdateProposalEvaluationScore(this.mainPanelObject).subscribe((data: any) => {
      this.mainPanelObject = data;
