...
import {
    CompleterOptions, ModuleDetails, ServiceRequest, ServiceRequestHistory,
...
export class RequestEditComponent implements OnInit, OnDestroy {
    ...
    private setUnitForNewRequest(): void {
        ...
        this.$subscriptions.push(
            this._overviewService.getPersonData(this._commonService.getCurrentUserDetail('personID')).subscribe((data: any) => {
                if (data.person.unit) {
                    this.selectDepartment(data.person.unit);
                    this.departmentSearchOptions.defaultValue = concatUnitNumberAndUnitName(data.person.unit.unitNumber,data.person.unit.unitName);
    ...
    saveOrUpdateServiceRequest(): void {
        if (!this.isSaving && this.validateSave()) {
            this.isSaving = true;
