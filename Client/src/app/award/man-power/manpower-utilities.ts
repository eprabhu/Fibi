...
/**
 * @param  {any} date
 * @param  {any} cutoffDate : Date value from parameter
 * checks if the date is same as cutoff date (used for salary for a resource in integration) and shows a warning
 */
export function checkCutoffDate(startDate: any, cutoffDate: any, isFromList: boolean = false): boolean {
  if (startDate) {
    startDate = isFromList ? getDateObjectFromTimeStamp(startDate) : new Date(startDate);
    return cutoffDate < new Date().getDate() && cutoffDate < startDate.getDate()
 ...
 * Checks if the resource added in existing position is overlapping the duration
 */
export function checkDuplicationExistingHire(categoryList: any, newResource: any, positionId: string) {
  let resourceArray: any = [];
  categoryList.forEach(category => {
    const duplication = resourceDuplicationExistingHire(category.awardManpowerResource, newResource, positionId);
    resourceArray = resourceArray.concat(duplication);
  });
  return resourceArray.length ? true : false;
}
/**
 * @param  {any} resourceList
 * @param  {any} newResource
 * @param  {string} positionId
 * checks and returns the resources in each category which are overlapping in duration for existing position
 */
function resourceDuplicationExistingHire(resourceList: any, newResource: any, positionId: string) {
  return resourceList.filter(resource => resource.manpowerResourceId !== newResource.manpowerResourceId &&
    resource.positionId && resource.positionId === positionId &&
    compareDatesForOverlap(resource.chargeStartDate ? resource.chargeStartDate : resource.planStartDate,
      resource.chargeEndDate ? resource.chargeEndDate : resource.planEndDate,
      newResource.chargeStartDate ? newResource.chargeStartDate : newResource.planStartDate,
      newResource.chargeEndDate ? newResource.chargeEndDate : newResource.planEndDate));
 ...
 * @param  {any} newResource
 * @param  {string} positionId
 * @param  {string} budgetRef
 * checks if resource with same position id is there in another budgetReferenceNumber and overlaps the new entry
 */
export function newExistingPositionOverlap(categoryList: any, newResource: any, positionId: string, budgetRef: string): boolean {
  let resourceOverlapping: any;
  categoryList.forEach(category => {
    if (budgetRef !== category.budgetReferenceNumber) {
      resourceOverlapping = resourceDuplicationExistingHire(category.awardManpowerResource, newResource, positionId);
    }
  });
  return resourceOverlapping.length ? true : false;
}
