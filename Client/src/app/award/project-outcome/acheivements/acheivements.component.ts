...
export class AcheivementsComponent implements OnInit, OnDestroy {
  ...
  */
  ...
  showSuccessToast() {
    const toastMsg = this.awardAcheivement.awardAcheivementAttachId ? 'Acheivement successfully updated.' :
      'Acheivement added successfully.';
  ...
  */
  ...
  deleteAcheivement(index, awardAcheivementAttachId) {
    ...
    this.$subscriptions.push(this._outcomeService.deleteAwardAcheivement({
      ...
      this._commonService.showToast(HTTP_SUCCESS_STATUS, 'Acheivement deleted successfully.');
    }));
  }
