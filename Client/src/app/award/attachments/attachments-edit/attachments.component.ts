...
export class AttachmentsEditComponent implements OnInit, OnDestroy {
  ...
  */
  ...
  showErrorToast() {
    let toastMsg;
    if (this.isReplaceAttachment) {
      toastMsg = !this._commonService.isWafEnabled ? 'Failed to replace attachment' : 'Waf blocked request for repalcing attachment';
    } else {
  ...
  */
  downloadAllAttachments() {
    ...
    this.$subscriptions.push(
      ...
      .subscribe((data: any) => {
        const filename = 'Project#' + this.awardData.awardId + '-Attachments.zip';
