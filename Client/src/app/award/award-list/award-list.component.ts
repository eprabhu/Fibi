...
export class AwardListComponent implements OnInit, OnDestroy {
	...
	ngOnInit() {
		...
		this.grantHttpOptions = this._awardListService.setHttpOptions('grantCallName', 'grantCallName', 'findGrantCall',
			'', { 'moduleCode': '1' });
	...
	checkForAdvanceSearch() {
		if (this.isAdvancedSearchMade() && this._navigationService.previousURL.includes('fibi/award')) {
			...
			this.clearAdvanceSearchFields(true);
	...
	setDefaultValueForCustomLibrarySearch() {
		...
		this.grantHttpOptions.defaultValue = this._awardListService.awardRequestExtraDetails.grantCallName || '';
	}
	...
	generateLookupArrayForDropdown() {
		...
		if (this._awardListService.awardRequestObject.property15.length !== 0) {
			...
			this.lookupValues['property11'] = [{ code: '3' }];    // Setting PI as default role, (code:3 for PI)
		}
	}
	...
	setGrantCallForAdvanceSearch(event) {
		this.tempAwardRequestObject.property10 = event ? event.grantCallId : null;
		this.tempAwardRequestObject.grantCallName = event ? event.grantCallName : '';
	}
	...
	clearAdvanceSearchFields(researchRoleValue) {
	...
	*/
	...
	clearAdvanceSearchList(researchRoleValue) {
		...
		if (researchRoleValue) {
			this.lookupValues['property11'] = [{ code: '3' }];
			this._awardListService.awardRequestObject.property11 = ['3'];
			this.tempAwardRequestObject.property11 = ['3'];  // Setting PI(code:3) as default role value
			this._awardListService.awardRequestObject.advancedSearch = 'A';
		} else {
