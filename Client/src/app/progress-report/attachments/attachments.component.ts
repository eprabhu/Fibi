...
export class AttachmentsComponent implements OnInit, OnDestroy {
    ...
    ngOnInit() {
        this.$subscriptions.push(this._commonData.getProgressReportData().subscribe((data: any) => {
            if (data && data.awardProgressReport) {
                ...
                this.checkEditMode(data.awardProgressReport.progressReportStatus.progressReportStatusCode);
   ...
   * For other users, there will not shows dropdown option for adding confidential attachments.*/
     ...
     * 6 = Completed
     */
    checkEditMode(progressReportStatusCode: string) {
        this.isAttachmentEdit = !(['4', '6'].includes(progressReportStatusCode));
    }
    ...
    showSuccessToast() {
        const toastMsg = this.isReplaceAttachment ? 'Attachment replaced succesfully' : 'Attachment added succesfully';
