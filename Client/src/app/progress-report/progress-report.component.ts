...
import { COMMON_APPROVE_LABEL, COMMON_RETURN_LABEL, DEFAULT_DATE_FORMAT, HTTP_ERROR_STATUS, HTTP_SUCCESS_STATUS } from '../app-constants';
...
export class ProgressReportComponent implements OnInit, OnDestroy {
    ...
    submitReport(): void {
        if (!this.isSaving) {
            ...
            this.$subscriptions.push(this._progressReportService.submitProgressReport(this.progressReportId).subscribe((res: any) => {
                ...
                this.showErrorMessage('Submiting Progress Report failed. Please try again.');
                this.isSaving = false;
                this._commonService.isShowOverlay = false;
            },
                () => {
    ...
    showSuccessToast(): void {
        if (this.requestObject.actionType === 'A') {
            ...
            this.showSuccessMessage(`Progress Report ${COMMON_RETURN_LABEL.toLowerCase()}d successfully.`);
        }
    }
    ...
    private validateViaServer(): void {
        ...
        if (!this.isSaving) {
            this.isSaving = true;
            this.$subscriptions.push((forkJoin(this._commonService.evaluateValidation(this.validationObject),this.validateQuestionnaire())).subscribe((res: any) => {
                this.isSaving = false;
