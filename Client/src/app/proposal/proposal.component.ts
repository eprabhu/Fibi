...
export class ProposalComponent implements OnInit, OnDestroy {
    ...
    checkSubmitProposalValidation() {
        if (this.dataVisibilityObj.dataChangeFlag ||
            (this._router.url.includes('budget') && this._budgetDataService.budgetDataChanged)) {
            this.dataVisibilityObj.isShowSaveBeforeExitWarning = true;
    ...
    continueToSubmit() {
        this._budgetDataService.budgetDataChanged = false;
        const tempReplace = this.dataVisibilityObj.dataChangeFlag;
        this.dataVisibilityObj.dataChangeFlag = false;
        this.checkSubmitProposalValidation();
        this.dataVisibilityObj.dataChangeFlag = tempReplace;
        this.updateDataVisibilityObj();
    }
    ...
    exportProposal() {
        ...
        this.$subscriptions.push(this._proposalService.printEntireProposal({ 'proposalId': this.result.proposal.proposalId })
            .subscribe(
                data => {
    ...
    /**setValidationMsgArray - sets questionnaire complete mesage, PI not present message and other messages from business rule
    ...
    */
    setValidationMsgArray() {
        ...
        } else if (this.validationMsg.length === 0 && this.isSubmitAction) {
            if (this.validationOnlyFlag) {
                $('#ValidateModal').modal('show');
            } else { this.showSubmitConfirmation(); }
        } else if (this.validationMsg.length === 0 && !this.isSubmitAction) {
    ...
    */
        ...
        if ((!this.result.enableClosedGrantCallLinkingInProposal) && (this.result.grantCall &&
            ...
            this.$subscriptions.push(this._budgetService.loadBudgetByProposalId({
                ...
                this.warningMsgObj.submitConfirmation = (this.budgetData.budgetHeader && !this.budgetData.isBudgetVersionEnabled && this.budgetData.budgetHeader.budgetStatusCode != '3') ?
                    'The Budget Status is In Progress. Do you want to make it Complete & proceed with submission?' : 'Are you sure you want to submit this proposal?';
            }));
        }
        this.updateDataVisibilityObj();
        this.isSubmitAction ? $('#warning-modal').modal('show') : $('#AdminCorrectionSubmitConfirmModal').modal('show');
    }
    ...
    canShowSave() {
        this.isShowSave = this.isEditModeAndRouteHasForms()
            ...
            || this.canShowSaveInrQuestionnaireTab();
    }

    private isEditModeAndRouteHasForms(): boolean {
        return this.dataVisibilityObj.mode === 'edit' && ['overview', 'other-information',
            'evaluation/evaluate', 'certification'].some(tabName => this._router.url.includes(tabName));
    }

    private canShowSaveInrQuestionnaireTab() {
		return this.dataVisibilityObj.mode === 'edit' || (this._router.url.includes('questionnaire') &&
            this.result.proposal && [3, 1, 9, 12].includes(this.result.proposal.statusCode) && this.result.availableRights.includes('MODIFY_PROPOSAL_QUESTIONNAIRE'));
	}
   ...
   */
    ...
    submitProposalOrAdminCorrection() {
        this.isSubmitAction ? this.continueToSubmit() : this.validateProposal();
    }
    ...
    createIP(): void {
        this.$subscriptions.push(this._proposalService.createIP({ 'proposalId': this.result.proposal.proposalId })
            .subscribe((data: any) => {
                ...
                this._commonService.showToast(HTTP_SUCCESS_STATUS, 'IP created Successfully.');
            }, err => {
                this._commonService.showToast(HTTP_ERROR_STATUS, 'Generating IP failed. Please try again.');
            }
            ));
    }
    ...
    checkLockBasedProposalMode(): void {
        if (this.dataVisibilityObj.mode === 'edit' && this.webSocket.isModuleLockAvailable === 'Available') {
            this.getLockForProposal();
        } else if (this.dataVisibilityObj.mode === 'edit' && this.webSocket.isModuleLockAvailable === 'NotAvailAble') {
            this.dataVisibilityObj.mode = 'view';
            this.getLockForProposal();
            this.webSocket.showModal();
        } else {
            this.dataVisibilityObj.mode = 'view';
        }
    }
