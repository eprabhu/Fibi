...
export class ToolKitComponent implements OnDestroy, OnInit {
    ...
    ngOnInit() {
        this.$subscriptions.push(this._toolKitService.getProposalHistoryInfo(this.route.snapshot.queryParamMap.get('proposalId'))
            .subscribe((result: any) => {
                ...
                this.filterSections();
                this.proposalVersionsData = this.formatProposalHistory(result.proposalHistory);
                this.masterVersion = this.setActiveVersion();
    ...
    formatProposalHistory(data: Array<any>): Array<History> {
    ...
    checkForComparisonVersion(data) {
        if (this.leftVersion) {
            this.rightVersion = this.leftVersion;
            this.leftVersion = this.setActiveVersion();
