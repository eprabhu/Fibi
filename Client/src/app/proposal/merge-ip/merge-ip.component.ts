...
export class MergeIpComponent implements OnChanges, OnDestroy {
        ...
        if (!this.isSaving && this.validateMerge()) {
            ...
            this.$subscriptions.push(this._merge.mergeIP(this.mergeRequest)
                .subscribe((data: string) => {
                    ...
                    this._commonService.showToast(HTTP_ERROR_STATUS, 'Merging IP failed. Please try again.');
                    this.isSaving = false;
                }
                ));
        }
    ...
    successfullyMergedIP(data: any): void {
        if (data && data.proposal) {
            ...
            this.resetValues();
            this._commonService.showToast(HTTP_SUCCESS_STATUS, 'IP merged Successfully.');
        } else {
