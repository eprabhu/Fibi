...
export class AttachmentComponent implements OnInit, OnDestroy {
  ...
  */
  setAttachmentEditableForAdmin() {
    this.result.dataVisibilityObj.isAttachmentEditable =  this._webSocket.isModuleLockAvailable === 'Available' ||  this._webSocket.isModuleLockAvailable === 'NotTriggered';
    this.isAdminEdit = this._webSocket.isModuleLockAvailable === 'Available' ||  this._webSocket.isModuleLockAvailable === 'NotTriggered';
      ...
      if (this.result.dataVisibilityObj.mode === 'view') {
        if (this.result.proposalAttachments.length > 0) {
          this.result.dataVisibilityObj.isAttachmentEditable = this.checkDepartmentLevelRightsInArray('MODIFY_PROPOSAL_ATTACHEMNTS') ?
            true : false;
            this.result.dataVisibilityObj.isAttachmentEditable =  this.result.dataVisibilityObj.isAttachmentEditable && (this._webSocket.isModuleLockAvailable === 'Available' ||  this._webSocket.isModuleLockAvailable === 'NotTriggered');
        } else {
