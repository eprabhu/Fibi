...
import {
    ...
    getEndPointOptionsForRolodexSearch,
    ...
    */
    ...
    validateOrganizationFields() {
        ...
        if (this.organizationObject.organizationTypeCode === '3' && !this.organizationObject.rolodexId) {
            this.map.set('rolodexName', 'rolodexName');
        }
        ...
        if (!this.isSaving) {
            ...
            this.$subscriptions.push(
                this._proposalHomeService.deleteProposalOrganization(this.orgDeleteId).subscribe(
                    () => {
                        const DELETEINDEX = this.organizationDetails.findIndex(ele => ele.proposalOrganizationId === this.orgDeleteId);
                        this.organizationDetails.splice(DELETEINDEX, 1);
                        this._commonService.showToast(HTTP_SUCCESS_STATUS, 'Organization deleted successfully');
                        this.isSaving = false;
                        if (this.editIndex) {
                            this.editIndex = this.organizationDetails.
                                findIndex(ele => ele.proposalOrganizationId === this.organizationObject.proposalOrganizationId);
                        }
                        this._dataStore.manualDataUpdate({ proposalOrganizations: this.organizationDetails });
                    },
                    (err) => {
    ...
    countrySelectFunction(event: any) {
        this.modalOrganizationObject.countryCode = event ? event.countryCode : null;
    }
    ...
    setUnsavedChanges(flag: boolean) {
        if (this.hasUnsavedChanges !== flag) {
            this._autoSaveService.setUnsavedChanges('Organization', 'Award104', flag);
        }
