    ...
    addSponsor() {
        ...
        if (this.fundingMap.size < 1) {
            ...
            this.$subscriptions.push(this._proposalHomeService.addFundingSupport({
                ...
                () => {
                    this._commonService.showToast(HTTP_SUCCESS_STATUS, 'Funding support ' + (!this.sponsorObject.sponsorId ? 'added' : 'updated') +
                        ' successfully.');
    ...
    deleteSponsor() {
        this.$subscriptions.push(this._proposalHomeService.deleteProposalSponsor({
            ...
            .subscribe((data: any) => {
                ...
                err => {
                    this._commonService.showToast(HTTP_ERROR_STATUS, 'Rmoving funding support failed. Please try again.');
                },
                () => {
