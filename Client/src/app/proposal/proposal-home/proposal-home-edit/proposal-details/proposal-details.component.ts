...
import { setFocusToElement, onKeyPress, removeUnwantedTags } from '../../../../common/utilities/custom-utilities';
...
export class ProposalDetailsComponent implements OnInit, OnDestroy, OnChanges {
    ...
    dataDependencies = ['proposal', 'isPINameAutoFilledRequired', 'piPersonId', 'rcbfTypeCode', 'isNonEmployee', 'availableRights',
        'enableActivityGrantCallMapping', 'grantEligibilityStatus', 'grantCall'];
    ...
    private setEndpointOptions() {
        ...
        this.unitHttpOptions = this.constructDepartmentHttpOptions(this.result.proposal.homeUnitName);
   ...
   */
     ...
     * using service _proposalHomeService.getPersonData loginned user details is fetched
    ...
    proposalPiIntialise() {
        if (this.result.isPINameAutoFilledRequired && !this.result.proposal.proposalId) {
            ...
            this.$subscriptions.push(this._proposalHomeService.getPersonData(this.result.piPersonId).subscribe((data: any) => {
                if (data.person.unit) {
                    this.checkUnitRight(data.person.unit.unitNumber, 'CREATE_PROPOSAL').then((isRightExists: boolean) => {
                        if (isRightExists) {
                            this.unitHttpOptions = this.constructDepartmentHttpOptions(data.person.unit.unitName);
    ...
    dateValidation() {
        ...
        if (this.result.proposal.sponsorDeadlineDate != null && this.result.proposal.internalDeadLineDate != null &&
            ...
            this.proposalDataBindObj.dateWarningList.set('internalDeadline',
                'Please choose an internal deadline date less than sponsor deadline date.');
        }
        this.deadlineDatesValidation();
        this.getDuration();
    }
    ...
    */
            ...
            if (value.unit_number) {
                this.checkUnitRight(value.unit_number, 'CREATE_PROPOSAL').then((isRightExists: boolean) => {
                    if (isRightExists) {
                        ...
                        this.unitHttpOptions.defaultValue = this.result.proposal.homeUnitName;
    ...
    constructDepartmentHttpOptions(unitName) {
        const httpOptions = getEndPointOptionsForLeadUnit(unitName);
        httpOptions.params = {
            check: 'all',
        ...
        if (this.dataVisibilityObj.dataChangeFlag) {
            if (isMandatoryFilled && !this.proposalDataBindObj.isGrantClosingDateError &&
                ...
                this.$subscriptions.push(this._proposalHomeService.saveProposalDetails({
                    ...
                    this._commonService.showToast(HTTP_ERROR_STATUS, 'Save proposalfailed. Please try again.');
                    this.isSaving = false;
   ...
   */
    ...
    listenForGlobalSave() {
        this.$subscriptions.push(this._autoSaveService.autoSaveTrigger$.subscribe(_saveClick => this.openDateChangeModal()));
    }
