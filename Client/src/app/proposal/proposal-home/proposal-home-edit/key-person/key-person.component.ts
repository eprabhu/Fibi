...
import { fileDownloader, setHelpTextForSubItems, validatePercentage } from '../../../../common/utilities/custom-utilities';
...
export class KeyPersonComponent implements OnInit, OnChanges, OnDestroy {
            ...
            this.$subscriptions.push(
                this._proposalService
                    ...
                    .subscribe((success: any) => {
                        ...
                        () => {
                            const toastMsg = 'Attachment added succesfully.';
   ...
   */
    ...
    */
        ...
        if (!this.isSaving) {
            ...
            this.$subscriptions.push(this._proposalHomeService.addKeyPerson({
                ...
                if (data.grantEligibilityStatus && data.grantEligibilityStatus.status === 'TRUE') {
                    ...
                    this._commonService.showToast(HTTP_SUCCESS_STATUS, (this.isEditKeyPerson) ?
                ...
                this._commonService.showToast(HTTP_ERROR_STATUS, (this.isEditKeyPerson) ?
    ...
    private getPersonReq(person) {
        return person.personId ?
            this._proposalHomeService.getPersonData(person.personId) :
            this._proposalHomeService.getRolodexData(person.rolodexId);
    }
    ...
    loadPersonnelAttachTypes() {
        if (!this.result.personnelAttachTypes) {
            this.$subscriptions.push(this._proposalHomeService
                .loadPersonnelAttachTypes()
                .subscribe((data: any) => {
                    this.result.personnelAttachTypes = data;
    ...
    setUnsavedChanges(flag: boolean) {
        if (this.hasUnsavedChanges !== flag) {
            this._autoSaveService.setUnsavedChanges('Key Personnel', 'proposal-keyperson-edit', flag);
        }
