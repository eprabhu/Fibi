...
export class ProjectTeamComponent implements OnInit, OnDestroy {
    ...
    */
     ...
     */
    maintainPersondetails(type, index) {
        ...
        if (this.proposalDataBindObj.mandatoryList.size <= 0 && !this.isSaving) {
            ...
            this.$subscriptions.push(this._proposalHomeService.addProjectTeam(this.personDetails).subscribe((data: any) => {
                if (type === 'I') {
                    ...
                    this.result.proposalProjectTeams.splice(index, 1, data.projectTeam);
                }
                ...
                err => { this._commonService.showToast(HTTP_ERROR_STATUS, 'Adding project team failed. Please try again.'); this.isSaving = false; }
            ));
        }
    }
    ...
    deletePerson(deleteindex) {
        ...
        this.$subscriptions.push(
            this._proposalHomeService.addProjectTeam((this.result.proposalProjectTeams.splice(deleteindex, 1)[0])).subscribe(data => {
                ...
                err => { this._commonService.showToast(HTTP_ERROR_STATUS, 'failed. Please try again.emoving project team failed. Please try again.'); },
    ...
    * */
    ...
    setUnsavedChanges(flag: boolean) {
        if (this.hasUnsavedChanges !== flag) {
            this._autoSaveService.setUnsavedChanges('Project Team', 'projectTeam', flag);
        }
