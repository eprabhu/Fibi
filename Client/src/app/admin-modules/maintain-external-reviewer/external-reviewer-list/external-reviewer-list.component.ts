...
export class ExternalReviewerListComponent implements OnInit, OnDestroy {
    ...
    sortCountObj = {
        ...
        'affilationInstitution.description': 0, 'countryDetails.countryName': 0, 'primaryEmail': 0, 'agreementStartDate': 0,
    ...
    async setAffiliatedInstitutionSearchOptions() {
        this.affiliatedInstitutionSearchOptions.arrayList = await this.lookUpData.extReviewerAffilation;
    ...
    onAffiliatedInstitutionSelect(event) {
        if (event) {
            this.tempExtRequestObject.property12 = event.affilationInstitutionCode;
        }else {
