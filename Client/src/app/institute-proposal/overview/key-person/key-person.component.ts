...
import { InstituteProposal, InstProposal, InstProposalPerson, PersonUnit,
	 ProposalPersonRole, Unit } from '../../institute-proposal-interfaces';
  ...
  personDescription: string;
	...
	removePreviousLeadUnit() {
		if (this.editIndex !== -1 && this.personDetails.isPi) {
			...
			const isLeadUnitChanged = this.personDetails.units.find(unit => unit.leadUnit).unitNumber === previousLeadUnit.unitNumber ? false : true; 
			const checkNewLeadUnitInList = this.personDetails.units.findIndex(unit => unit.propPersonUnitId === previousLeadUnit.propPersonUnitId) >= 0 ? true : false;
