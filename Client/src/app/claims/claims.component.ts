        ...
        .subscribe((data: any) => {
          ...
          err => {
            ...
            this._commonService.showToast(HTTP_ERROR_STATUS,
                `${COMMON_APPROVE_LABEL.toLowerCase()}/disapprove claim faield. Please try again.`);
            this.isSaving = false;
            this._commonService.isShowOverlay = false;
          },
          () => {
  ...
  */
  ...
  showSuccessToast() {
    if (this.requestObject.actionType === 'A') {
      ...
      this._commonService.showToast(HTTP_SUCCESS_STATUS, `Claim ${COMMON_RETURN_LABEL.toLowerCase()}d successfully.`);
    }
  }
      ...
      this.$subscriptions.push(
        this._claimsService.exportClaimReport({ 'claimId': this.claimId })
          .subscribe((data: any) => {
            ...
            this.isSaving = false;
          }, err => { this.isSaving = false; this._commonService.showToast(HTTP_ERROR_STATUS, 'Claim report downloading failed. Please try again.'); }));
    }
  }
