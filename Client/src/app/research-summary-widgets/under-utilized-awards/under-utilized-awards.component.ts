...
export class UnderUtilizedAwardsComponent extends GoogleChartService implements OnInit, OnDestroy {
  ...
  setUtilisationData(data) {
    const HIGH_UTILISATION = data.widgetDatas.find(item => item.includes('High Utilisation'));
    const LOW_UTILISATION = data.widgetDatas.find(item => item.includes('Low Utilisation'));
    const MEETS_UTILISATION = data.widgetDatas.find(item => item.includes('Acceptable Utilisation'));
    if (HIGH_UTILISATION) {
      this.underUtilizedAwards.push(HIGH_UTILISATION);
      this.colors.push('#109618');
    }
    if (LOW_UTILISATION) {
      this.underUtilizedAwards.push(LOW_UTILISATION);
      this.colors.push('#DC3912');
    }
    if (MEETS_UTILISATION) {
      this.underUtilizedAwards.push(MEETS_UTILISATION);
