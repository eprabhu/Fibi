...
export class ResearchSummaryWidgetsComponent implements OnInit, OnDestroy {
	...
	private getRootUnit() {
		const ROOT_UNIT = this.unitAdministrators.find(ele => ele.parentUnitNumber == null);
		this.categoryCompleterOptions.defaultValue = ROOT_UNIT && ROOT_UNIT.unitDetail;
		this.updateUnitChange(ROOT_UNIT);
	}
	...
	public updateWidgets() {
		this._researchSummaryConfigService.selectedUnit$.next({
		...this.unitObject, ...{'descentFlag': this.isDescentFlagOn ? 'Y' : 'N'}
		});
	}
