...
import { compareDates, parseDateWithoutTimestamp, removeTimeZoneFromDateObject } from '../common/utilities/date-utilities';
...
export class ExternalReviewComponent implements OnInit, OnDestroy {
        ...
        if (this.checkMandatoryFilled() && !this.isSaving) {
            ...
            this.$subscriptions.push(this._reviewService
                .saveExternalReview({
                    ...
                    err => {
                        this.isSaving = false;
                        this._commonService.showToast(HTTP_ERROR_STATUS, 'Crete Review failed. Please try again.');
                    }));
        }
    }
    ...
    closeModal() {
        ...
        this.mandatoryList.clear();
        this.addExternalReviewObject.extReviewServiceTypeCode = null;
        this.addExternalReviewObject.deadlineDate = null;
        this.addExternalReviewObject.description = null;
        this.deadlineDateValidation();
    }
    ...
    sendReview() {
        this.$subscriptions.push(this._reviewService.sendReview(this.sendForReviewExtReviewID).subscribe((data: any) => {
            ...
            this._commonService.showToast(HTTP_SUCCESS_STATUS, 'Review sent successfully');
        },
            err => {
                this._commonService.showToast(HTTP_ERROR_STATUS, 'Send for Review failed. Please try again.');
            }));
    }
