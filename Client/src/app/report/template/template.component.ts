...
import { getValuesForBirt } from '../birt-value-builder';
  ...
  generateBirtReport(reportType: string) {
    if (!this.isSaving) {
      if (this.checkMandatoryFilterFields()) {
        ...
        this._commonService.showToast(HTTP_ERROR_STATUS, 'Please fill all mandatory criteria to generate report');
      }
    }
  }
 ...
 */
   ...
   */
  openNewTemplate(id) {
    this._router.navigate(['fibi/dashboard/report/template'], { queryParams: { id: id, tc: this.currentModule } });
  }
  ...
  checkMandatoryFilterFields() {
    ...
    this.filtersFieldsData.filters.forEach(F => {
      if (F.isMandatory && ((Array.isArray(this.filterValues[F.columnName]) && !this.filterValues[F.columnName].length) ||
        (!Object.keys(this.filterValues[F.columnName]).length) || !this.filterValues[F.columnName])) {
