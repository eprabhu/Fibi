...
import { getEndPointOptionsForAwardNumber, getEndPointOptionsForDepartment, getEndPointOptionsForLeadUnit, getEndPointOptionsForProposalDisclosure, getEndPointOptionsForSponsor } from '../../../../../fibi/src/app/common/services/end-point.config';
			...
			if (!this.canReviseFCOI) {
				this.$subscriptions.push(this._disclosureCreateModalService.reviseDisclosure(this.reviseObject)
					.subscribe((data: any) => {
						...
						this._router.navigate(['/coi/disclosure/summary'], { queryParams: { disclosureId: data.coiDisclosure.disclosureId } });
						this.clearModal();
					}, err => {
	...
	createProjectDisclosureAPI(): void {
		if (this.validateProject()) {
			this.$subscriptions.push(this._disclosureCreateModalService.createDisclosure(this.getCreateProjectRequestObject()).subscribe((data: any) => {
				if (data) {
					this._router.navigate(['/coi/create-disclosure/screening'], { queryParams: { disclosureId: data.coiDisclosure.disclosureId } });
					this.clearModal();
				}
			}, err => {
				this.commonService.showToast(HTTP_ERROR_STATUS, (err.error && err.error.errorMessage) ?
