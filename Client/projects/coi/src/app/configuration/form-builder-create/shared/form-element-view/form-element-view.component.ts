...
export class FormElementViewComponent implements OnInit {
 ...
 */
  setDefaultValues(customElement) {
    ...
    setLookUpOptions(customElement) {
        this.lookUpOptions = customElement.columnName+'#'+customElement.lookupArgument+'#true#true';
    }
 ...
 */
  ...
  checkValidation(customField, event, list, id) {
      ...
      if (list.dataType === 'NE' && event.keyCode >= 65 && event.keyCode <= 90) {
          this.isType = true;
