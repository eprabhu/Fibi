...
public class ConsultingDisclosureDaoImpl implements ConsultingDisclosureDao {
	...
	@Override
	public Timestamp savePersonEntityDetails(Integer personEntityId, Integer disclosureId, Integer entityNumber) {
		Timestamp timeStamp = commonDao.getCurrentTimestamp();
        StringBuilder hqlQuery = new StringBuilder();
        Session session = hibernateTemplate.getSessionFactory().getCurrentSession();
        hqlQuery.append("UPDATE ConsultingDisclosure d SET d.personEntityId = :personEntityId, d.entityNumber = :entityNumber, "); 
        hqlQuery.append("d.updateTimeStamp = :updateTimestamp, d.updateUser = :updateUser ");
        hqlQuery.append("WHERE d.disclosureId = :disclosureId");
        Query query = session.createQuery(hqlQuery.toString());
        query.setParameter("disclosureId", disclosureId);
