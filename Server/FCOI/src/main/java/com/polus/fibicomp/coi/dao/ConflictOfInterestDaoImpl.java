...
//		hqlQuery.append("INNER JOIN COI_DISC_DETAILS_COMMENTS C4 ON C4.DISCLOSURE_DETAILS_ID=C2.DISCLOSURE_DETAILS_ID ");
		hqlQuery.append("WHERE C2.CONFLICT_STATUS_CODE IS NOT NULL AND C1.PERSON_ID = :personId ");
	...
	public DashBoardProfile getCOIDashboard(CoiDashboardVO vo) {
		...
		try {
			...
			while (resultSet.next()) {
				...
				disclosureView.setConflictStatusCode(Integer.parseInt(resultSet.getString("CONFLICT_STATUS_CODE")));
				disclosureView.setConflictStatus(resultSet.getString("DISCLOSURE_STATUS"));
				disclosureView.setDispositionStatusCode(Integer.parseInt(resultSet.getString("DISPOSITION_STATUS_CODE")));
				disclosureView.setDispositionStatus(resultSet.getString("DISPOSITION_STATUS"));
				disclosureView.setSubmittedDate(resultSet.getTimestamp("CERTIFIED_AT"));
...
//				disclosureView.setPersonId(resultSet.getString("PERSON_ID"));
	...
	public DashBoardProfile getCOIAdminDashboard(CoiDashboardVO vo) {
		...
		try {
			...
			while (resultSet.next()) {
				...
				disclosureView.setConflictStatusCode(Integer.parseInt(resultSet.getString("DISCLOSURE_STATUS_CODE")));
				disclosureView.setConflictStatus(resultSet.getString("DISCLOSURE_STATUS"));
				disclosureView.setDispositionStatusCode(Integer.parseInt(resultSet.getString("DISPOSITION_STATUS_TYPE_CODE")));
				disclosureView.setDispositionStatus(resultSet.getString("DISPOSITION_STATUS"));
				disclosureView.setFcoiTypeCode(resultSet.getString("DISCLOSURE_CATEGORY_TYPE_CODE"));
				disclosureView.setFcoiType(resultSet.getString("DISCLOSURE_CATEGORY_TYPE"));
				disclosureView.setReviewStatusCode(resultSet.getString("REVIEW_STATUS_TYPE_CODE"));
				disclosureView.setReviewStatus(resultSet.getString("REVIEW_STATUS"));
				...
				disclosureView.setVersionStatus(resultSet.getString("DISCLOSURE_SEQUENCE_STATUS_CODE"));
	...
	private Integer getCOIAdminDashboardCount(CoiDashboardVO vo) {
		...
		try {
			...
			throw new ApplicationException("Error in getCOIAdminDashboardCount {}", e, Constants.JAVA_ERROR);
		}
		return count;
	}
