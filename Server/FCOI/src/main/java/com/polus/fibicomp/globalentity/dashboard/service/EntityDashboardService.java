...
public class EntityDashboardService {
	...
	private String getErrorMessage(String key, String value) {
		...
		switch (key) {
			case "PRIMARY_NAME":
				return validateFormat(value, "[\",<>'_()/\\.\\[\\]\\{\\}&#@:0-9a-zA-Z -]*+", "Entity Name must not include special characters.");
			...
			case "PRIMARY_ADDRESS_LINE_1":
				return validateFormat(value, "[\",<>'_()/\\.\\[\\]\\{\\}&#@:0-9a-zA-Z -]*", "Address Line 1 must not include special characters.");
			case "PRIMARY_ADDRESS_LINE_2":
				return validateFormat(value, "[\",<>'_()/\\.\\[\\]\\{\\}&#@:0-9a-zA-Z -]*", "Address Line 2 must not include special characters.");
			case "CITY":
				return validateFormat(value, "[0-9a-zA-Z ()]+", "City must not include special characters.");
			case "STATE":
				return validateFormat(value, "[0-9a-zA-Z _(),&.*-]+", "State must not include special characters.");
			case "COUNTRY_CODE":
				return validateFormat(value, "[0-9a-zA-Z _]+", "Country must not include special characters.");
			case "DUNS_NUMBER":
				return validateFormat(value, "[0-9, ]*", "Duns Number must not include special characters.");
			case "UEI_NUMBER":
				return validateFormat(value, "[0-9a-zA-Z _-]+", "UEI Number must not include special characters.");
			case "CAGE_NUMBER":
				return validateFormat(value, "[0-9a-zA-Z. _()-/]+", "CAGE Number must not include special characters.");
			case "WEBSITE_ADDRESS":
				return validateFormat(value, "[\",<>_()/\\.\\[\\]\\{\\}&#@:0-9a-zA-Z -]+", "Website must not include special characters.");
			...
			case "PRIOR_NAME":
				return validateFormat(value, "[\",<>_()/\\.\\[\\]\\{\\}&#@:0-9a-zA-Z -]*", "Prior Name must not include special characters.");
			case "SORT_TYPE":
