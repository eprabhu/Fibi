...
public interface FormBuilderSectionComponentEntityRepository  extends JpaRepository<FormBuilderSectionComponentEntity, Integer> {
	...
	@Modifying
	@Query(value = "DELETE FROM FORM_BUILDER_SECTION_COMPONENT WHERE FORM_BUILDER_SECTION_ID = ?1", nativeQuery = true)
    void deleteBySectionId(Integer formBuilderSectId);
