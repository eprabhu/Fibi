			...
			dtos = jdbcTemplate.execute((Connection conn) -> {
				try (CallableStatement cs = conn.prepareCall("{call COI_INT_GET_ENTITY_DETAILS(?)}")) {
					...
					try (ResultSet rset = cs.executeQuery()) {
						...
						while (rset.next()) {
							results.add(EntityDTO.builder().certifiedEmail(rset.getString("CERTIFIED_EMAIL"))
									...
									.sponsorState(rset.getString("SPONSOR_STATE")).orgCity(rset.getString("ORG_CITY"))
									...
									.orgState(rset.getString("ORG_STATE")).primaryName(rset.getString("PRIMARY_NAME"))
									...
									.ueiNumber(rset.getString("UEI_NUMBER")).dodacNumber(rset.getString("DODAC_NUMBER"))
									.dunsNumber(rset.getString("DUNS_NUMBER")).acronym(rset.getString("ACRONYM"))
									...
									.riskLevel(rset.getString("RISK_LEVEL")).createdBy(rset.getString("CREATED_BY"))
