...
public class CleanseMatchUrlBuilder {
	...
	public String buildApiUrl(DnBEntityCleanseMatchRequestDTO entityMatch) {
		...
		if (entityMatch.getSourceDunsNumber() != null && !entityMatch.getSourceDunsNumber().isEmpty()) {
			return new UrlBuilder(baseUrl).addParam("duns", entityMatch.getSourceDunsNumber()).build();
		} else {
			return new UrlBuilder(baseUrl).addParam("name", entityMatch.getSourceDataName())
