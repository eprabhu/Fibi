...
CREATE TABLE `coi_discl_project_entity_rel` (
  ...
  `COI_DISCL_PROJECTS_ID` INT NULL,
  `PERSON_ENTITY_ID` INT NULL,
  `PERSON_ENTITY_NUMBER` INT NULL,
  `PREVIOUS_PERSON_ENTITY_ID` INT NULL,
  `ENTITY_ID` INT NULL,
  `PROJECT_CONFLICT_STATUS_CODE` VARCHAR(10) NULL,
  `UPDATED_BY` VARCHAR(40) NULL,
  `UPDATE_TIMESTAMP` DATETIME NULL,
  PRIMARY KEY (`COI_DISCL_PROJECT_ENTITY_REL_ID`),
  INDEX `COI_DISCL_PROJECT_ENTITY_REL_FK1_idx` (`COI_DISCL_PROJECTS_ID` ASC) VISIBLE,
  INDEX `COI_DISCL_PROJECT_ENTITY_REL_FK2_idx` (`PERSON_ENTITY_ID` ASC) VISIBLE,
  INDEX `COI_DISCL_PROJECT_ENTITY_REL_FK3_idx` (`ENTITY_ID` ASC) VISIBLE,
  INDEX `COI_DISCL_PROJECT_ENTITY_REL_FK4_idx` (`PROJECT_CONFLICT_STATUS_CODE` ASC) VISIBLE,
  CONSTRAINT `COI_DISCL_PROJECT_ENTITY_REL_FK1`
    FOREIGN KEY (`COI_DISCL_PROJECTS_ID`)
    REFERENCES `coi_discl_projects` (`COI_DISCL_PROJECTS_ID`)
    ON DELETE NO CASCADE
    ON UPDATE NO CASCADE,
  CONSTRAINT `COI_DISCL_PROJECT_ENTITY_REL_FK2`
    FOREIGN KEY (`PERSON_ENTITY_ID`)
    REFERENCES `person_entity` (`PERSON_ENTITY_ID`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `COI_DISCL_PROJECT_ENTITY_REL_FK3`
    FOREIGN KEY (`ENTITY_ID`)
    REFERENCES `entity` (`ENTITY_ID`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `COI_DISCL_PROJECT_ENTITY_REL_FK4`
    FOREIGN KEY (`PROJECT_CONFLICT_STATUS_CODE`)
    REFERENCES `coi_proj_conflict_status_type` (`PROJECT_CONFLICT_STATUS_CODE`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION);