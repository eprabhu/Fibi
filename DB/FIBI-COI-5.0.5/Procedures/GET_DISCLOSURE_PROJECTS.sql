...
DECLARE LS_PERSON_ID VARCHAR(40);
        ...
        SELECT DISTINCT
            ...
            LEFT JOIN (SELECT T5.COI_DISCL_PROJECT_ENTITY_REL_ID , T6.MODULE_ITEM_KEY,
						T6.MODULE_CODE, MAX(T5.PROJECT_CONFLICT_STATUS_CODE) AS PROJECT_CONFLICT_STATUS_CODE
						FROM COI_DISCL_PROJECT_ENTITY_REL T5
						INNER JOIN COI_DISCL_PROJECTS T6 ON T6.COI_DISCL_PROJECTS_ID = T5.COI_DISCL_PROJECTS_ID
						WHERE T6.DISCLOSURE_ID= AV_DISCLOSURE_ID AND T6.MODULE_CODE = 1
						GROUP BY T6.MODULE_ITEM_KEY, T6.MODULE_CODE )T4 ON T4.MODULE_ITEM_KEY = T2.EXTERNAL_SYSTEM_REF_ID
            LEFT JOIN (SELECT MODULE_ITEM_KEY,
						MODULE_CODE, COUNT(C1.PERSON_ENTITY_ID) AS RELATIONSHIP_COUNT
						...
						WHERE C2.DISCLOSURE_ID= AV_DISCLOSURE_ID
						AND C2.MODULE_CODE = 1
						AND C1.PERSON_ENTITY_ID IS NOT NULL
            ...
            LEFT JOIN (SELECT CONCAT('[', GROUP_CONCAT(CONCAT('{', CONFLICT_STATUS_GROUP, ': ', PROJECT_COUNT, '}')), ']') AS CONFLICT_STATUS_COUNT,
						MODULE_ITEM_KEY FROM ( SELECT CASE
											...
											AND C1.MODULE_CODE = 1 GROUP BY CONFLICT_STATUS_GROUP, C1.MODULE_ITEM_KEY ) AS grouped
		...
		T2.LEAD_UNIT_NAME AS HOME_UNIT_NAME,
            ...
            INNER JOIN EPS_PROP_PERSON_ROLE T3 ON T2.KEY_PERSON_ROLE_CODE = T3.PROP_PERSON_ROLE_ID
            LEFT JOIN (SELECT T5.COI_DISCL_PROJECT_ENTITY_REL_ID ,
						T6.MODULE_ITEM_KEY,
						T6.MODULE_CODE,
						MAX(T5.PROJECT_CONFLICT_STATUS_CODE) AS PROJECT_CONFLICT_STATUS_CODE
						FROM COI_DISCL_PROJECT_ENTITY_REL T5
						INNER JOIN COI_DISCL_PROJECTS T6 ON T6.COI_DISCL_PROJECTS_ID = T5.COI_DISCL_PROJECTS_ID
						WHERE T6.DISCLOSURE_ID= AV_DISCLOSURE_ID
						AND T6.MODULE_CODE = 3
						GROUP BY T6.MODULE_ITEM_KEY, T6.MODULE_CODE )T4 ON T4.MODULE_ITEM_KEY = T2.EXTERNAL_SYSTEM_REF_ID
            LEFT JOIN (SELECT T6.MODULE_ITEM_KEY,
						T6.MODULE_CODE,
                        ...
                        GROUP BY T6.MODULE_ITEM_KEY, T6.MODULE_CODE )T5 ON T5.MODULE_ITEM_KEY = T2.EXTERNAL_SYSTEM_REF_ID
            LEFT JOIN COI_PROJ_CONFLICT_STATUS_TYPE T6 ON T6.PROJECT_CONFLICT_STATUS_CODE = T4.PROJECT_CONFLICT_STATUS_CODE
            LEFT JOIN (SELECT COUNT(*) AS IS_PROJECT_COMPLETED_COUNT,
						...
						AND C1.DISCLOSURE_ID = AV_DISCLOSURE_ID
                        AND C1.MODULE_CODE = 3 GROUP BY C1.MODULE_ITEM_KEY ) T7 ON T7.MODULE_ITEM_KEY = T2.EXTERNAL_SYSTEM_REF_ID
            LEFT JOIN (SELECT CONCAT('[', GROUP_CONCAT(CONCAT('{', CONFLICT_STATUS_GROUP, ': ', PROJECT_COUNT, '}')), ']') AS CONFLICT_STATUS_COUNT,
                            MODULE_ITEM_KEY FROM ( SELECT CASE
                                    WHEN C2.PROJECT_CONFLICT_STATUS_CODE BETWEEN 100 AND 199 THEN 1
                                    WHEN C2.PROJECT_CONFLICT_STATUS_CODE BETWEEN 200 AND 299 THEN 2
                                    WHEN C2.PROJECT_CONFLICT_STATUS_CODE BETWEEN 300 AND 399 THEN 3
									END AS CONFLICT_STATUS_GROUP, COUNT(*) AS PROJECT_COUNT , C1.MODULE_ITEM_KEY
									FROM COI_DISCL_PROJECT_ENTITY_REL C2
									INNER JOIN COI_DISCL_PROJECTS C1 ON C1.COI_DISCL_PROJECTS_ID = C2.COI_DISCL_PROJECTS_ID
									WHERE C2.PROJECT_CONFLICT_STATUS_CODE IS NOT NULL
									AND C1.DISCLOSURE_ID = AV_DISCLOSURE_ID
									AND C1.MODULE_CODE = 3
									GROUP BY CONFLICT_STATUS_GROUP, C1.MODULE_ITEM_KEY
									) AS grouped
							GROUP BY MODULE_ITEM_KEY) T8 ON T8.MODULE_ITEM_KEY = T2.EXTERNAL_SYSTEM_REF_ID
            LEFT JOIN COI_PROJECT_TYPE T9 ON T9.COI_PROJECT_TYPE_CODE = T2.COI_PROJECT_TYPE_CODE
			 ...
			 T2.PROPOSAL_START_DATE AS START_DATE,
					...
					T2.LEAD_UNIT_NAME AS HOME_UNIT_NAME,
                        ...
                        LEFT JOIN (SELECT T5.COI_DISCL_PROJECT_ENTITY_REL_ID ,
									...
									FROM COI_DISCL_PROJECT_ENTITY_REL T5
									INNER JOIN COI_DISCL_PROJECTS T6 ON T6.COI_DISCL_PROJECTS_ID = T5.COI_DISCL_PROJECTS_ID
									WHERE T6.DISCLOSURE_ID= AV_DISCLOSURE_ID AND T6.MODULE_CODE = 2 GROUP BY T6.MODULE_ITEM_KEY, T6.MODULE_CODE )T4 ON T4.MODULE_ITEM_KEY = T2.EXTERNAL_SYSTEM_REF_ID
                        LEFT JOIN (SELECT T6.MODULE_ITEM_KEY,
									T6.MODULE_CODE,
									COUNT(T5.PERSON_ENTITY_ID) AS RELATIONSHIP_COUNT
                                    FROM COI_DISCL_PROJECT_ENTITY_REL T5
                        ...
                        LEFT JOIN (SELECT COUNT(*) AS IS_PROJECT_COMPLETED_COUNT,
                                    ...
                                    WHERE C2.PROJECT_CONFLICT_STATUS_CODE IS NULL
									AND C1.DISCLOSURE_ID = AV_DISCLOSURE_ID
                                    AND C1.MODULE_CODE = 2 GROUP BY C1.MODULE_ITEM_KEY ) T7 ON T7.MODULE_ITEM_KEY = T2.EXTERNAL_SYSTEM_REF_ID
                        LEFT JOIN (SELECT CONCAT('[', GROUP_CONCAT(CONCAT('{', CONFLICT_STATUS_GROUP, ': ', PROJECT_COUNT, '}')), ']') AS CONFLICT_STATUS_COUNT,
                                        MODULE_ITEM_KEY FROM ( SELECT CASE
																WHEN C2.PROJECT_CONFLICT_STATUS_CODE BETWEEN 100 AND 199 THEN 1
																WHEN C2.PROJECT_CONFLICT_STATUS_CODE BETWEEN 200 AND 299 THEN 2
