...
BEGIN
		...
		WHEN OTHERS THEN
			...
			LS_SPONSOR_ERROR_MESGE := 'Error fetching update user: ' || SQLERRM;
		END;	
		...
		IF AV_ORGANIZATION_ID IS NULL THEN 
            ...
            -- SELECT (MAX(NVL(ROLODEX_ID,0))+1) INTO LI_ORG_SEQ_ROLODEX_ID FROM ROLODEX;
		...
		IF (UPPER(IS_SPONSOR_SYNC_FLAG) = 'TRUE' OR IS_SPONSOR_SYNC_FLAG = 1) THEN 
			  ...
			  SELECT SEQ_ROLODEX_ID.NEXTVAL INTO LI_SPON_SEQ_ROLODEX_ID FROM dual;
               ...
               -- SELECT (MAX(NVL(ROLODEX_ID,0))+1) INTO LI_SPON_SEQ_ROLODEX_ID FROM ROLODEX;
