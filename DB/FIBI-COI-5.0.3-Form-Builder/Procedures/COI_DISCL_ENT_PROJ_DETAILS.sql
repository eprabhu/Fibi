...
SET LS_QRY_CONDITIONS ='';
    ...
    ELSEIF AV_MODULE_CODE IS NOT NULL AND AV_MODULE_ITEM_KEY IS NOT NULL THEN
            SET LS_QRY_CONDITIONS = CONCAT(' AND T10.MODULE_CODE = ', AV_MODULE_CODE, ' AND T10.MODULE_ITEM_KEY = ''',AV_MODULE_ITEM_KEY,'''' );
    ...
    SET LS_DYN_SQL = CONCAT('SELECT DISTINCT
									...
									T8.VERSION_NUMBER,
									T8.VERSION_STATUS,
									T8.ENTITY_STATUS_CODE,
									T8.CREATE_USER AS ENTITY_CREATE_USER,
									T8.CREATE_TIMESTAMP AS ENTITY_CREATE_TIMESTAMP,
									T8.UPDATE_USER AS ENTITY_UPDATE_USER,
									T8.UPDATE_TIMESTAMP AS ENTITY_UPDATE_TIMESTAMP,
									T8.RISK_CATEGORY_CODE,
									T11.COUNTRY_NAME AS ENTITY_COUNTRY_NAME,
									T12.DESCRIPTION AS ENTITY_TYPE,
									...
									COALESCE(T5.BADGE_COLOR, T6.BADGE_COLOR, T7.BADGE_COLOR) AS BADGE_COLOR,
									T9.DESCRIPTION AS PROJECT_CONFLICT_STATUS
								FROM COI_DISCL_PROJECT_ENTITY_REL T1
								INNER JOIN COI_DISCL_PROJECTS T10 ON T10.COI_DISCL_PROJECTS_ID = T1.COI_DISCL_PROJECTS_ID
								LEFT JOIN COI_PROJECT_AWARD_V T2 ON T2.EXTERNAL_SYSTEM_REF_ID = T10.MODULE_ITEM_KEY AND T10.MODULE_CODE = 1
								LEFT JOIN COI_PROJECT_PROPOSAL_V T3 ON T3.EXTERNAL_SYSTEM_REF_ID = T10.MODULE_ITEM_KEY AND T10.MODULE_CODE = 3
								LEFT JOIN coi_project_institute_proposal_v T4 ON T4.EXTERNAL_SYSTEM_REF_ID = T10.MODULE_ITEM_KEY AND T10.MODULE_CODE = 2
								...
								INNER JOIN COUNTRY T11 ON T11.COUNTRY_CODE = T8.COUNTRY_CODE
								INNER JOIN ENTITY_TYPE T12 ON T12.ENTITY_TYPE_CODE = T8.ENTITY_TYPE_CODE
								LEFT JOIN COI_PROJ_CONFLICT_STATUS_TYPE T9 ON T9.PROJECT_CONFLICT_STATUS_CODE = T1.PROJECT_CONFLICT_STATUS_CODE
								WHERE T10.DISCLOSURE_ID = ', AV_DISCLOSURE_ID, LS_QRY_CONDITIONS,
