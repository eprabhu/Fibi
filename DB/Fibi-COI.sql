...
CREATE TABLE `discl_valid_compnent_comnt_ty` (
  ...
  CONSTRAINT `DISCL_VALID_CMPNENT_COMNT_TY_FK2` FOREIGN KEY (`COMMENT_TYPE`) REFERENCES `discl_comment_type` (`COMMENT_TYPE_CODE`)
);
...
ADD CONSTRAINT `OPA_DISCLOSURE_FK4`
  ...
  ON DELETE NO ACTION
  ON UPDATE NO ACTION;
UPDATE `COI_REVIEW_STATUS_TYPE` SET `DESCRIPTION` = 'Review Assigned' WHERE (`REVIEW_STATUS_CODE` = '7');
UPDATE `COI_REVIEW_STATUS_TYPE` SET `DESCRIPTION` = 'Review In Progress' WHERE (`REVIEW_STATUS_CODE` = '3');
