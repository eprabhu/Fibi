...
IF AV_TAB_TYPE = 'IN_PROGRESS_DISCLOSURES' THEN
		...
		IF AV_FILTER_TYPE = 'ALL' THEN
                ...
                SET TAB_QUERY = CONCAT(' AND T1.FCOI_TYPE_CODE IN (1,2,3) AND T1.CONFLICT_STATUS_CODE IN (1,2,4,5,6) 
...
ELSEIF AV_TAB_TYPE = 'APPROVED_DISCLOSURES' THEN
		...
		IF AV_FILTER_TYPE = 'ALL' THEN
				...
				SET TAB_QUERY = CONCAT(' AND T1.FCOI_TYPE_CODE IN (1,2,3) AND T1.CONFLICT_STATUS_CODE = 3 
...
ELSEIF AV_TAB_TYPE = 'TRAVEL_DISCLOSURES' THEN
                ...
                SET TAB_QUERY = CONCAT(' AND T1.FCOI_TYPE_CODE = 4 AND T1.VERSION_STATUS IN (1,2) ');
								
ELSEIF AV_TAB_TYPE = 'DISCLOSURE_HISTORY' THEN

				SET TAB_QUERY = CONCAT(' AND T1.VERSION_STATUS = 3 ');
...
END IF;
        ...
        SET LS_DYN_SQL =CONCAT('SELECT DISTINCT *  FROM(SELECT DISTINCT
										...
										INNER JOIN COI_CONFLICT_STATUS_TYPE T2 ON T2.CONFLICT_STATUS_CODE=T1.CONFLICT_STATUS_CODE
