...
SET LS_DYN_SQL ='';
	...
	SET LS_DYN_SQL = CONCAT('SELECT * FROM (SELECT DISTINCT T1.ENTITY_ID, T2.PERSON_ENTITY_ID, T1.ENTITY_NAME, T2.PERSON_ID, T5.COUNTRY_NAME, 
    	T3.DESCRIPTION AS ENTITY_TYPE, group_concat(r.DISCLOSURE_TYPE,'': ['', r.RELATIONSHIP, '']''
    	ORDER BY r.DISCLOSURE_TYPE ASC SEPARATOR '','') AS RELATIONSHIPS, T4.DESCRIPTION AS RISK, T1.ENTITY_NUMBER,
        ...
        where PERSON_ID = ',AV_PERSON_ID,' ))T ', LS_FILTER_CONDITION, ' ORDER BY CASE WHEN T.PERSON_ENTITY_ID IS NULL THEN 1 ELSE 0 END LIMIT 20') ;
