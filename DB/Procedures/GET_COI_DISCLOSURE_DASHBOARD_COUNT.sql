...
ELSEIF AV_TAB_TYPE = 'TRAVEL_DISCLOSURES' THEN
                ...
                IF AV_FILTER_TYPE = 'ALL' THEN
					SET TAB_QUERY = CONCAT(' AND T21.VERSION_STATUS IN (''ACTIVE'',''PENDING'') ');
...
IF AV_TAB_TYPE = 'TRAVEL_DISCLOSURES' THEN
		SET LS_DYN_SQL = CONCAT(' SELECT DISTINCT COUNT(*) FROM(SELECT DISTINCT
							...
							T21.TRAVEL_DISCLOSURE_STATUS,
							T21.VERSION_STATUS,
                            T21.REVIEW_STATUS,
                            T21.DISPOSITION_STATUS,
							...
							T23.FULL_NAME AS PERSON_NAME ' ,SELECTED_FIELD_LIST,'
							...
							LEFT JOIN PERSON T23 ON T23.PERSON_ID=T21.PERSON_ID WHERE T21.PERSON_ID = ',AV_PERSON_ID,TAB_QUERY,
										' GROUP BY T21.TRAVEL_DISCLOSURE_ID)T ',LS_FILTER_CONDITION,' ',AV_SORT_TYPE);
ELSE
        SET LS_DYN_SQL =CONCAT('SELECT DISTINCT COUNT(*)  FROM(SELECT DISTINCT
										T1.DISCLOSURE_ID,
