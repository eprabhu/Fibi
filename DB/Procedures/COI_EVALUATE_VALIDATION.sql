...
BEGIN
  ...
  INNER JOIN PERSON_ENTITY_RELATIONSHIP T3 ON T1.PERSON_ENTITY_ID = T3.PERSON_ENTITY_ID
  INNER JOIN VALID_PERSON_ENTITY_REL_TYPE T4 ON T3.VALID_PERS_ENTITY_REL_TYP_CODE = T4.VALID_PERS_ENTITY_REL_TYP_CODE
  WHERE T1.PERSON_ID = AV_PERSON_ID 
  AND T1.VERSION_STATUS = 'PENDING'
  AND T4.DISCLOSURE_TYPE_CODE = 1;
            
  IF LI_SFI_FLAG THEN

    SET LS_SFI_LIST = '';
    SELECT GROUP_CONCAT(DISTINCT CONCAT(t1.PERSON_ENTITY_ID, '||', t2.ENTITY_NAME) SEPARATOR ':;:') INTO LS_SFI_LIST
    FROM PERSON_ENTITY t1
    INNER JOIN Entity t2 ON t1.ENTITY_ID = t2.ENTITY_ID
	INNER JOIN PERSON_ENTITY_RELATIONSHIP T3 ON T1.PERSON_ENTITY_ID = T3.PERSON_ENTITY_ID
	INNER JOIN VALID_PERSON_ENTITY_REL_TYPE T4 ON T3.VALID_PERS_ENTITY_REL_TYP_CODE = T4.VALID_PERS_ENTITY_REL_TYP_CODE
    WHERE t1.PERSON_ID = AV_PERSON_ID  AND t1.VERSION_STATUS = 'PENDING' AND T4.DISCLOSURE_TYPE_CODE = 1;
