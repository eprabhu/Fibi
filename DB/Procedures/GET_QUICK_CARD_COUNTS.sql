...
ELSEIF AV_TYPE = 'REVIEWER' THEN
	SET LS_DYN_SQL =CONCAT('SELECT (SELECT COUNT(DISTINCT T1.DISCLOSURE_ID)  FROM COI_DISCLOSURE T1 
					...
					LEFT JOIN COI_REVIEW T2 ON T2.DISCLOSURE_ID = T1.DISCLOSURE_ID LEFT JOIN COI_DISCLOSURE_DETAILS T3 ON T3.DISCLOSURE_ID = T1.DISCLOSURE_ID 
                    WHERE T3.DISC_DET_STATUS_CODE IN (1,3) AND T2.ASSIGNEE_PERSON_ID = ',AV_PERSON_ID, ') AS CONFLICTS_IDENTIFIED');
END IF;
