...
IF AV_TYPE IS NOT NULL AND AV_TYPE <> '' AND  AV_TYPE = 'PERSON' THEN 
    ...
    SET TAB_QUERY = CONCAT(TAB_QUERY,' WHERE T1.ENTITY_ID =',AV_ENTITY_ID, ' AND T1.VERSION_STATUS = ''ACTIVE'' ');

END IF;

IF AV_TYPE IS NOT NULL AND AV_TYPE <> '' AND  AV_TYPE = 'FINANCIAL_DISCLOSURES' THEN 

    SET TAB_QUERY = CONCAT(TAB_QUERY,' WHERE T13.ENTITY_ID =',AV_ENTITY_ID, ' AND T1.VERSION_STATUS = ''ACTIVE'' ');
...
SET TAB_QUERY = CONCAT(TAB_QUERY,' WHERE T21.ENTITY_ID =',AV_ENTITY_ID, ' AND T21.VERSION_STATUS = ''ACTIVE'' ');
END IF;
