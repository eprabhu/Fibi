...
ELSEIF AV_TAB_TYPE = 'PENDING_DISCLOSURES' THEN
                ...
                SET SELECTED_FIELD_LIST= CONCAT(SELECTED_FIELD_LIST,' ,T23.COI_REVIEW_ID, T23.DESCRIPTION as REVIEW_DESCRIPTION, T23.REVIEW_STATUS_CODE AS REVIEWER_STATUS_CODE, T24.DESCRIPTION AS REVIEWER_STATUS, IFNULL(T25.ADMIN_GROUP_NAME,T26.FULL_NAME) AS REVIEWER_NAME ');
                ...
                SET TAB_QUERY = CONCAT(TAB_QUERY,' ',LS_ADMIN_GROUP_CONDITION ,' OR T23.ASSIGNEE_PERSON_ID = ''',AV_PERSON_ID,'''  AND T23.REVIEW_STATUS_CODE <>3)');
				SET JOIN_CONDITION =  CONCAT(JOIN_CONDITION,' INNER JOIN COI_REVIEW T23 ON T23.DISCLOSURE_ID = T1.DISCLOSURE_ID
                INNER JOIN COI_REVIEW_STATUS T24 ON T24.REVIEW_STATUS_CODE = T23.REVIEW_STATUS_CODE
...
ELSEIF AV_TAB_TYPE = 'FCOI_DISCLOSURES' THEN
                ...
                SET TAB_QUERY = CONCAT(' AND FCOI_TYPE_CODE = ''1'' ');
...
/* ELSEIF AV_TAB_TYPE = 'TRAVEL_DISCLOSURES' THEN
                ...
                SET TAB_QUERY = CONCAT(' AND FCOI_TYPE_CODE = ''4'' '); */

ELSEIF AV_TAB_TYPE = 'ALL_REVIEWS' THEN

				SET TAB_QUERY = CONCAT(' AND T1.REVIEW_STATUS_CODE IN (2,3)');
...
END IF;
        ...
        SET LS_DYN_SQL = CONCAT('SELECT DISTINCT *  FROM(SELECT DISTINCT
										...
										LEFT JOIN DISPOSITION_STATUS_TYPE T3 ON T3.DISPOSITION_STATUS_CODE = T1.DISPOSITION_STATUS_CODE
										...
										JOIN_CONDITION, ' WHERE T1.VERSION_STATUS IN (1,2) ', TAB_QUERY, 
