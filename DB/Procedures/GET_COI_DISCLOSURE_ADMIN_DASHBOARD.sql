...
IF AV_TAB_TYPE = 'ALL_DISCLOSURES' THEN
                ...
                SET LS_ADMIN_GROUP_FCOI_CONDITION = CONCAT(' AND (T1.DISCLOSURE_ID IN(SELECT DISCLOSURE_ID FROM COI_DISCLOSURE WHERE 
																	''', AV_PERSON_ID ,''' IN (SELECT T54.PERSON_ID FROM PERSON_ROLES T54 INNER JOIN ADMIN_GROUP T55 ON T54.ROLE_ID = T55.ROLE_ID WHERE T55.MODULE_CODE=8) OR
																	ADMIN_GROUP_ID IN(
				...
				SET LS_ADMIN_GROUP_PROJECT_CONDITION = CONCAT(' AND (T1.DISCLOSURE_ID IN(SELECT DISCLOSURE_ID FROM COI_DISCLOSURE WHERE 
																	''', AV_PERSON_ID ,''' IN (SELECT T54.PERSON_ID FROM PERSON_ROLES T54 INNER JOIN ADMIN_GROUP T55 ON T54.ROLE_ID = T55.ROLE_ID WHERE T55.MODULE_CODE=8) OR 
                                                                    ADMIN_GROUP_ID IN(
																	SELECT ADMIN_GROUP_ID FROM ADMIN_GROUP 
																	WHERE ROLE_ID IN (SELECT T51.ROLE_ID FROM ROLE_RIGHTS T51 
...
ELSEIF AV_TAB_TYPE = 'NEW_SUBMISSIONS' THEN
				...
				SET LS_ADMIN_GROUP_FCOI_CONDITION = CONCAT(' AND (EXISTS(SELECT T51.ROLE_ID FROM ROLE_RIGHTS T51 
                                                                    ...
                                                                    INNER JOIN ADMIN_GROUP T55 ON T54.ROLE_ID = T55.ROLE_ID WHERE T55.MODULE_CODE=8)))');	
...
ELSEIF AV_TAB_TYPE = 'ALL_REVIEWS' THEN
				...
				SET TAB_QUERY = CONCAT(TAB_QUERY,' AND T1.DISPOSITION_STATUS_CODE = 1 AND T1.REVIEW_STATUS_CODE IN (3,7) AND T1.VERSION_STATUS = ''PENDING''');
...
ELSEIF AV_TAB_TYPE = 'MY_REVIEWS' THEN
				...
				SET TAB_QUERY = CONCAT(TAB_QUERY,' AND T1.REVIEW_STATUS_CODE IN (3,7) 
				AND ((''', AV_PERSON_ID ,''' IN (SELECT T54.PERSON_ID FROM PERSON_ROLES T54 INNER JOIN ADMIN_GROUP T55 ON T54.ROLE_ID = T55.ROLE_ID WHERE T55.MODULE_CODE=8)) OR
				(''', AV_PERSON_ID ,''' IN (SELECT T34.ASSIGNEE_PERSON_ID WHERE T34.REVIEW_STATUS_TYPE_CODE IN (1,3))))
...
END IF;
	...
	SET JOIN_CONDITION =  CONCAT(' LEFT JOIN COI_CONFLICT_STATUS_TYPE T2 ON T2.CONFLICT_STATUS_CODE=T1.CONFLICT_STATUS_CODE
        ...
        SET LS_DYN_SQL = CONCAT('SELECT DISTINCT *  FROM(SELECT DISTINCT ',SELECTED_FIELD_LIST,'
