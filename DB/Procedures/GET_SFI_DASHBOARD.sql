...
IF AV_TYPE IS NOT NULL AND AV_TYPE <> '' AND  AV_TYPE = 'A' THEN 
	IF AV_ENTITY_NAME IS NOT NULL  AND AV_ENTITY_NAME <> '' THEN
		SET LS_FILTER_CONDITION = CONCAT(LS_FILTER_CONDITION,' T.COI_ENTITY_NAME LIKE ''%',AV_ENTITY_NAME, '%'' AND ');
	END IF;
...
END IF;
        ...
        SET LS_DYN_SQL = CONCAT('SELECT DISTINCT *  FROM (SELECT DISTINCT
										T1.COI_FINANCIAL_ENTITY_ID, 
										T1.ENTITY_VERSION_NUMBER, 
										T2.COI_ENTITY_NAME, 
										...
										FROM COI_FINANCIAL_ENTITY T1 
										LEFT JOIN COI_ENTITY T2 ON T2.COI_ENTITY_ID = T1.COI_ENTITY_ID 
                                        LEFT JOIN (SELECT T1.COI_FINANCIAL_ENTITY_ID, COUNT(DISTINCT T1.DISCLOSURE_ID) AS NO_OF_DISCLOSURES 
												   FROM COI_DISCLOSURE_DETAILS T1 INNER JOIN COI_DISCLOSURE T2 ON T2.DISCLOSURE_ID = T1.DISCLOSURE_ID 
                                                   WHERE T2.DISCLOSURE_SEQUENCE_STATUS_CODE IN (''1'',''2'') 
                                                   GROUP BY T1.COI_FINANCIAL_ENTITY_ID) T3 ON T3.COI_FINANCIAL_ENTITY_ID = T1.COI_FINANCIAL_ENTITY_ID 
										LEFT JOIN (SELECT T1.COI_FINANCIAL_ENTITY_ID, COUNT(DISTINCT T1.MODULE_ITEM_KEY) AS NO_OF_PROPOSALS 
												   FROM COI_DISCLOSURE_DETAILS T1 WHERE T1.MODULE_CODE = 3 
                                                   GROUP BY T1.COI_FINANCIAL_ENTITY_ID) T4 ON T4.COI_FINANCIAL_ENTITY_ID = T1.COI_FINANCIAL_ENTITY_ID 
										LEFT JOIN (SELECT T1.COI_FINANCIAL_ENTITY_ID, COUNT(DISTINCT T1.MODULE_ITEM_KEY) AS NO_OF_AWARDS 
												   FROM COI_DISCLOSURE_DETAILS T1 WHERE T1.MODULE_CODE = 1 
                                                   GROUP BY T1.COI_FINANCIAL_ENTITY_ID) T5 ON T5.COI_FINANCIAL_ENTITY_ID = T1.COI_FINANCIAL_ENTITY_ID 
                                        ...
                                        ' GROUP BY T1.COI_FINANCIAL_ENTITY_ID) T ',LS_FILTER_CONDITION,' ',AV_SORT_TYPE,' ',LS_OFFSET_CONDITION);
