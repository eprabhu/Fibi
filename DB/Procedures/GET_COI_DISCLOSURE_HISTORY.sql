...
BEGIN
    ...
    IF AV_FILTER_TYPE = 'ALL' THEN
		...
		SET DISC_TAB_QUERY = CONCAT(' AND T1.VERSION_STATUS IN (''ARCHIVE'',''ACTIVE'') ');
        SET TRAVEL_TAB_QUERY = CONCAT(' AND T21.VERSION_STATUS IN (''ARCHIVE'',''ACTIVE'') ');
                                
	ELSEIF AV_FILTER_TYPE = 'FCOI' THEN
        
		SET DISC_TAB_QUERY = CONCAT(' AND T1.FCOI_TYPE_CODE = 1 AND T1.VERSION_STATUS IN (''ARCHIVE'',''ACTIVE'') ');
    ...
    IF AV_FILTER_TYPE = 'ALL' OR AV_FILTER_TYPE = 'FCOI' OR AV_FILTER_TYPE = 'PROJECT' THEN
		...
		SET DISC_JOIN_CONDITION = CONCAT(' FROM COI_DISCLOSURE T1 INNER JOIN COI_DISCLOSURE_FCOI_TYPE T2 ON T2.FCOI_TYPE_CODE = T1.FCOI_TYPE_CODE
                                    ...
                                    INNER JOIN COI_CONFLICT_STATUS_TYPE T4 ON T4.CONFLICT_STATUS_CODE = T1.CONFLICT_STATUS_CODE
	...
	IF AV_FILTER_TYPE = 'ALL' OR AV_FILTER_TYPE = 'TRAVEL' THEN
		...
		SET TRAVEL_SELECTED_FIELD_LIST= CONCAT('NULL AS DISCLOSURE_ID, T21.TRAVEL_DISCLOSURE_ID, T21.UPDATE_TIMESTAMP, T21.VERSION_STATUS, NULL AS  FCOI_TYPE_CODE,
                                        NULL AS FCOI_TYPE, NULL AS HOME_UNIT, NULL AS UNIT_NAME, T21.EXPIRATION_DATE,  NULL AS CERTIFIED_AT, NULL AS CONFLICT_STATUS_CODE, 
    ...
    IF AV_FILTER_TYPE = 'ALL' THEN
		SET LS_DYN_SQL = CONCAT('SELECT * FROM (SELECT DISTINCT ',DISC_SELECTED_FIELD_LIST, DISC_JOIN_CONDITION, DISC_TAB_QUERY, ' UNION SELECT DISTINCT ',
							TRAVEL_SELECTED_FIELD_LIST, TRAVEL_DISC_JOIN_CONDITION, TRAVEL_TAB_QUERY, ' ) T ORDER BY UPDATE_TIMESTAMP DESC ');
    
    ELSEIF AV_FILTER_TYPE = 'TRAVEL' THEN
		SET LS_DYN_SQL = CONCAT('SELECT DISTINCT ',TRAVEL_SELECTED_FIELD_LIST, TRAVEL_DISC_JOIN_CONDITION, TRAVEL_TAB_QUERY, ' ORDER BY T21.UPDATE_TIMESTAMP DESC');
    
    ELSE
		SET LS_DYN_SQL = CONCAT('SELECT DISTINCT ',DISC_SELECTED_FIELD_LIST, DISC_JOIN_CONDITION, DISC_TAB_QUERY, ' ORDER BY T1.UPDATE_TIMESTAMP DESC');
    END IF;
