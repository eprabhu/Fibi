...
BEGIN
    ...
    */
    ...
    ELSEIF AV_TAB_TYPE = 'MY_REVIEWS' THEN
        ...
        SET LS_FILTER_CONDITION = CONCAT(' T1.OPA_DISCLOSURE_STATUS_CODE IN (3,7,8) AND ((T8.ASSIGNEE_PERSON_ID = ''', AV_PERSON_ID ,'''  
                    AND T8.REVIEW_STATUS_TYPE_CODE IN (1,2)) 
                    OR (T1.ADMIN_PERSON_ID = ''', AV_PERSON_ID ,''')) ');

    ELSEIF AV_TAB_TYPE = 'ALL_REVIEWS' THEN

        SET LS_FILTER_CONDITION = CONCAT(' T1.OPA_DISCLOSURE_STATUS_CODE IN (3,7,8) ');
        SET LS_FILTER_CONDITION = CONCAT(LS_FILTER_CONDITION, ' AND( T1.ADMIN_PERSON_ID = ''', AV_PERSON_ID ,'''
                    OR (T1.OPA_DISCLOSURE_ID IN(SELECT OPA_DISCLOSURE_ID FROM OPA_DISCLOSURE WHERE (ADMIN_GROUP_ID IN(
    ...
    IF AV_IS_FACULTY IS NOT NULL AND AV_IS_FACULTY <> '' THEN 
        SET LS_FILTER_CONDITION = CONCAT(LS_FILTER_CONDITION,' AND T1.IS_FACULTY = ''',AV_IS_FACULTY,''' ');
    END IF;
    ...
    SET OPA_JOINS = CONCAT(' INNER JOIN OPA_CYCLES T2 ON T2.OPA_CYCLE_NUMBER = T1.OPA_CYCLE_NUMBER 
        ...
        LEFT JOIN opa_review_status_type T14 ON T14.REVIEW_STATUS_CODE = T8.REVIEW_STATUS_TYPE_CODE
