...
INNER JOIN FIBI_COI_QNR_MAPPING T2 ON T2.SOURCE_QNR_ID = T1.QUESTIONNAIRE_ID
INNER JOIN FIBI_COI_QNR_QSTN_MAPPING T3 ON T3.SOURCE_QSTN_ID = T1.QUESTION_ID
WHERE PROPOSAL_NUMBER = AV_PROPOSAL_NUMBER
AND KEY_PERSON_ID = AV_PERSON_ID
AND QUESTIONNAIRE_ID = AV_QUESTIONNAIRE_ID
AND UPPER(TRIM(ANSWER)) = 'YES';
