...
ELSE
	...
	-- Check if the project (IP) is linked to an award. If yes, hierarchy data is fetched based on the award number.
        SELECT T2.AWARD_NUMBER INTO LS_PROJECT_NUMBER
        ...
        WHERE PROPOSAL_NUMBER = AV_PROJECT_NUMBER ORDER BY T1.UPDATE_TIMESTAMP LIMIT 1;
	...
	ELSEIF AV_MODULE_CODE = 3 THEN
        ...
        WHERE T4.PROPOSAL_ID = AV_PROJECT_NUMBER ORDER BY T1.UPDATE_TIMESTAMP LIMIT 1;
		
        IF LS_PROJECT_NUMBER IS NULL THEN 
		
			-- Check if proposal has linked IP , If exists then hierarchy data is fetched based ip number
			SELECT DISTINCT T1.IP_NUMBER INTO LS_PROJECT_NUMBER
	...
	IF LS_MODULE_CODE = 1 THEN
        ...
        WHERE (AWARD_SEQUENCE_STATUS = 'ACTIVE' OR (AWARD_SEQUENCE_STATUS = 'PENDING' AND T1.SEQUENCE_NUMBER= 1))
		AND T1.AWARD_NUMBER = LS_PROJECT_NUMBER;

    ELSEIF LS_MODULE_CODE = 2 THEN
        
        SELECT DISTINCT IFNULL(T4.AWARD_NUMBER,NULL) AS AWARD_NUMBER,IFNULL(T1.PROPOSAL_NUMBER,NULL) AS IP_NUMBER,IFNULL(T1.PROPOSAL_ID,NULL) AS PROPOSAL_NUMBER
        FROM PROPOSAL T1
        ...
        WHERE (EXISTS (SELECT 1 
               FROM PROPOSAL P 
               WHERE P.PROPOSAL_NUMBER = T1.PROPOSAL_NUMBER 
                 AND P.PROPOSAL_SEQUENCE_STATUS = 'PENDING') 
       OR T1.PROPOSAL_SEQUENCE_STATUS = 'ACTIVE') 
