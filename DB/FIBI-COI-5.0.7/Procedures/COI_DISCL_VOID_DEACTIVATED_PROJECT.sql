...
AV_REMARK	 		VARCHAR(40)
)
BEGIN

	IF AV_PERSON_ID IS NOT NULL THEN
		UPDATE COI_DISCLOSURE
        SET DISPOSITION_STATUS_CODE = 2, REMARK = AV_REMARK
        WHERE FCOI_TYPE_CODE = 2 AND PERSON_ID = AV_PERSON_ID;
	ELSE
		UPDATE COI_DISCLOSURE t1 INNER JOIN COI_DISCL_PROJECTS t2 ON t2.DISCLOSURE_ID = t1.DISCLOSURE_ID
