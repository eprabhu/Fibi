...
ALTER TABLE `coi`.`coi_review_comment_tags` 
DROP FOREIGN KEY `COI_REVIEW_CMT_TAGS_FK2`;
ALTER TABLE `coi`.`coi_review_comment_tags` 
ADD CONSTRAINT `COI_REVIEW_CMT_TAGS_FK2`
  FOREIGN KEY (`COI_REVIEW_COMMENT_ID`)
  REFERENCES `coi`.`discl_comment` (`COMMENT_ID`)
  ON DELETE CASCADE;

INSERT INTO `coi`.`DISCL_COMMENT_TYPE` (`COMMENT_TYPE_CODE`, `DESCRIPTION`, `IS_ACTIVE`, `UPDATE_TIMESTAMP`, `UPDATE_USER`) VALUES ('1', 'Application Comment', 'Y', now(), 'quickstart');
